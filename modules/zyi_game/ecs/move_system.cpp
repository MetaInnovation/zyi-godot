...
template <typename T>
int64_t ZyiMoveSystemInternalComponentPool<T>::acquire_component() {
...
void ZyiMoveSystem::idle_process_update(double p_delta) {
	...
	for (ZyiNormalMoveComponent &component : normal_move_component_pool.pool) {
		node = component.get_valid_move_node();
	...
	CharacterBody2D *character_body = nullptr;
	for (ZyiCharacterMoveComponent &component : character_move_component_pool.pool) {
		character_body = component.get_valid_move_node();
		if (!character_body) {
			continue;
		}
		bool need_update_boids_grid = component.is_in_boid_grid() && boids_grid.is_valid();
...
void ZyiMoveSystem::idle_physics_process_update(double p_delta) {
	...
	for (ZyiNormalMoveComponent &component : normal_move_component_pool.pool) {
		node = component.get_valid_move_node();
		if (!component.check_can_auto_update_velocity() || !node || !node->is_inside_tree()) {
			continue;
		}
	...
	for (ZyiKnockbackMoveComponent &component : knockback_move_component_pool.pool) {
		node = component.get_valid_move_node();
		if (!component.moving || !node || !node->is_inside_tree()) {
			continue;
		}
	...
	for (ZyiCharacterMoveComponent &component : character_move_component_pool.pool) {
		character_body = component.get_valid_move_node();
		if ((!component.check_can_auto_update_velocity() && !component.knockback_moving) || !character_body || !character_body->is_inside_tree()) {
			continue;
		}
